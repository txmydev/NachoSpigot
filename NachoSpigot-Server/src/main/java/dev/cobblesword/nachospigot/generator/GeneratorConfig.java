package dev.cobblesword.nachospigot.generator;
...
public class GeneratorConfig {
    ...
    public float mineshaftMultiplier;
    ...
    public GeneratorConfig(String worldName) {
        ...
        List<BiomeBase> spawnBiomes = new ArrayList<>();
        int spawnBiomePlainsChance = getInt("spawn.biome.plains", 50);
        int spawnBiomeDesertChance = getInt("spawn.biome.desert", 30);
        int spawnBiomeSavannaChance = getInt("spawn.biome.savanna", 20);
        for(int i = 0; i < spawnBiomePlainsChance; i++)
            spawnBiomes.add(BiomeBase.PLAINS);
        for(int i = 0; i < spawnBiomeDesertChance; i++)
            spawnBiomes.add(BiomeBase.DESERT);
        for(int i = 0; i < spawnBiomeSavannaChance; i++)
            spawnBiomes.add(BiomeBase.SAVANNA);
        int random = (int) (Math.random() * spawnBiomes.size());
    ...
    private BiomeBase getBiome(String name) {
        for (BiomeBase biome : BiomeBase.getBiomes()) {
            if (biome.ah.equalsIgnoreCase(name)) {
                return biome;
            }
        }
